...
celery_beat_schedule = {
    ...
    'monthly-report': {
        ...
        'schedule': crontab(hour=9, minute=0, day_of_month=1),  # 9 AM on 1st of every month
    },
}
...
def daily_reminder():
    ...
    try:
        ...
        new_lots = ParkingLot.query.filter(
            ParkingLot.created_at >= week_ago
        ).all()
        ...
        for user in inactive_users:
            ...
            if new_lots:
                ...
                lots_info = "\n".join([f"- {lot.prime_location_name} at {lot.address}" for lot in new_lots])
                message += f"\n\nNew locations:\n{lots_info}"
            else:
            ...
            print(f"Reminder sent to {user.username}: Email={email_sent}, Chat={chat_sent}")
...
def monthly_report():
    ...
    try:
        ...
        for user in users:
            ...
            total_spent = sum([r.parking_cost or 0 for r in reservations if r.status == 'completed'])
            ...
            total_duration = sum([
                r.get_duration_hours() for r in reservations 
                if r.status == 'completed' and r.parking_timestamp and r.leaving_timestamp
            ])
...
def export_user_data_csv(user_id, email):
    ...
    try:
        ...
        writer.writerow([
            ...
            'Reservation Date', 'Parking Start', 'Parking End', 'Duration (Hours)',
            'Cost ($)', 'Status', 'Remarks'
        ])
        ...
        for reservation in reservations:
            writer.writerow([
                ...
                reservation.parking_timestamp.strftime('%Y-%m-%d %H:%M:%S') if reservation.parking_timestamp else '',
