...
class ParkingSpot(db.Model):
    ...
    status = db.Column(
        db.String(1), nullable=False, default="A"
...
class Reservation(db.Model):
    ...
    def calculate_cost(self):
        ...
        if self.parking_timestamp and self.leaving_timestamp:
            ...
            duration_hours = (
                self.leaving_timestamp - self.parking_timestamp
            ).total_seconds() / 3600
    ...
    def get_duration_hours(self):
        ...
        if self.parking_timestamp and self.leaving_timestamp:
            return round(
                (self.leaving_timestamp - self.parking_timestamp).total_seconds()
                / 3600,
                2,
            )
        elif self.parking_timestamp:
            return round(
                (datetime.utcnow() - self.parking_timestamp).total_seconds() / 3600, 2
            )
        return 0.0
    ...
    def to_dict(self):
        ...
        return {
            ...
            "reservation_timestamp": (
                self.reservation_timestamp.isoformat()
